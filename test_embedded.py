...
import numpy as np
import random
from util.loss import normalize as normalize_
######
...
def rotating_matrix(x, y):
    ...
    v = normalize_(y - torch.cos(theta) * x)
...
def Get_test_results_single(model, expInfo, is_before_linear=False, flip=True, print_features = False, feature_center = None):
    model.train(False)

    gallery_eq_query = True
    feature_size = expInfo.feature_size

    query_path = expInfo.image_datasets['test'].imgs
    ...
    # Extract feature
    recall_ks = []
    query_feature = extract_feature(model, expInfo, dataloaders=expInfo.dataloaders['test'], is_before_linear = is_before_linear, flip=flip)
    ...
    query_feature_mean_not_normed = normalize(query_feature_mean_not_normed)

    intra, inter = get_intra_inter(expInfo, query_feature, query_feature_mean_not_normed, query_label)
    ...
    if print_features:
        if feature_center is None:
            get_center = True
            feature_center = normalize_(torch.mean(query_feature, dim=0))
        origin = torch.zeros(feature_size)
        origin[feature_size - 1] = 1.0
        rot = rotating_matrix(feature_center, origin)

        print('test features')
        for ind in range(expInfo.class_nb):
            mask = (query_label == ind)
            if expInfo.computer == '58':
                mask = torch.from_numpy(np.array(mask, dtype=np.uint8))
            features_single_class = query_feature[mask]
            rotted_feature = torch.mm(features_single_class.cuda(), rot.t())
            print(ind)
            for vec in rotted_feature:
                print_values(vec, name='')

    # recall_ks_n = []
    # query_feature_n = extract_feature_n(model, dataloaders, feature_size, is_before_linear = is_before_linear, flip=flip)

    sim_mat = pairwise_similarity(query_feature)
    # sim_mat_n = pairwise_similarity(query_feature_n)
    ...
    if gallery_eq_query is True:
        ...
        # sim_mat_n = sim_mat_n - torch.eye(sim_mat_n.size(0))

    if expInfo.data_type == 'SOP':
        recall_ks.append(Recall_at_ks(sim_mat, query_ids=query_label, gallery_ids=gallery_label, data='product'))
    else:
        recall_ks.append(Recall_at_ks(sim_mat, query_ids=query_label, gallery_ids=gallery_label, data='cub'))
        # recall_ks_n.append(Recall_at_ks(sim_mat_n, query_ids=query_label, gallery_ids=gallery_label, data='cub'))

    # print(recall_ks)
    # print(recall_ks_n)

    if print_features and get_center:
        return recall_ks[0], feature_center
    else:
        return recall_ks[0], intra, inter

def Get_test_results_inshop(model, expInfo):
    feature_size = expInfo.feature_size

    query_path = expInfo.image_datasets['query'].imgs
    gallery_path = expInfo.image_datasets['gallery'].imgs
    ...
    # Extract feature
    query_feature = extract_feature(model, expInfo.dataloaders['query'], feature_size)
    gallery_feature = extract_feature(model, expInfo.dataloaders['gallery'], feature_size)
...
def extract_feature(model, expInfo, dataloaders, is_before_linear=False, flip=True):
    ...
    for data in dataloaders:
        ...
        if is_before_linear:
            ff = torch.FloatTensor(n, expInfo.original_size).zero_()
        else:
            ff = torch.FloatTensor(n, expInfo.feature_size).zero_()
        ...
        for i in range(ind):
            ...
            if is_before_linear:
                f, _ = model(input_img, get_feature_before_linear=True)
            else:
                f, _ = model(input_img)
...
def extract_feature_n(model, dataloaders, feature_size, is_before_linear=False, flip=True):
    n_param = 5000
